...
def subtract_background(data,sigma=3.0,title=''):
...
class ModelFitter():
    def __init__(self,data,model,n_norm=10,verbose=True):
        self.data = data
        self.model = model
    ...
    def fit_helper(self,standardized_params,fixed_params,fixed_idx,free_idx,param_template):
    ...
    def fit_model(self,bounds,N_iter=4,rtol_init=1e-1,rtol_iter=1e-2,
        ...
        print('* Free params:',self.free_params)
        print('* Fixed params:',self.fixed_params)
        ...
        # initial fit
        print(f'* Starting iteration 1/{N_iter}...')
        print('* x0 =',x0_physical)
    ...
    def plot_result(self,vmin=None,vmax=None,cmap='viridis',title=''):
        if vmin is None and vmax is None:
            vmin,vmax = np.nanpercentile(self.data,[0.1,99.9])
        elif vmin is None:
            vmin = self.data.min()
        elif vmax is None:
